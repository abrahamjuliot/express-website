...
server
    ...
    .get('/:name', (req, res) => {                  // 4b. get '/:name'
        var source = main[req.sourceName];
        if (source) {res.render(source);}
        else {err404(req, res);}
    })
    .get('/food/:name', (req,res) => {             // 4b. get 'bikes/:name'
    ...
    .listen(process.env.PORT, () => {               // 5. listen on port
